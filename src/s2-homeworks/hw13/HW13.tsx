  ...
  const send = (x?: boolean | null) => () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setInfo(res.data.info || "Ваш запрос успешно выполнен");
        // дописать
      })
      .catch((e) => {
        if (e.response) {
          if (e.response.status === 400) {
            setCode("Ошибка 400!");
            setImage(error400);
            setText(e.response.data.errorText || "Некорректный запрос");
            setInfo(
              e.response.data.info || "Проверьте данные и повторите запрос"
            );
